% QCD_test2: Tests the quadrature based implementation of r(FOM)2 as a recycling method
% for the special case where the matrix A remains fixed and only the right hand sides change. 

% The example used is the sign function of a Hermitian QCD matrix of size 49,152 x 49,152
...
p.m = 250;       % m: Length of Arnoldi cycle
p.k = 40;        % k: Dimension of recycling subspace
...
for ix=1:num_systems
    ...
    fprintf("\n Relative Residual (ar) of Arnoldi approximation %f\n",arnoldi_resid(ix) );

    rfom2_resid(ix) = norm(frr - b)/norm(b);
    fprintf("\n Relative Residual (rr) of r(FOM)^2 approximation %f\n",rfom2_resid(ix));
